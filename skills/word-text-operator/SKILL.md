...
读取整篇文档的纯文本。直接调用即可，不需要先调用其他查询方法。
...
在文档中查找所有匹配项，返回位置列表 `[{start, end, text}, ...]`。直接调用即可，不需要先调用 `get_full_text`。

```python
{"action": "find_all", "params": {"text": "关键词"}, "description": "查找所有匹配"}
```

#### 6. count_occurrences **[查询]**


统计关键词在文档中出现的次数。直接调用即可，不需要先调用 `get_full_text`。
...
使用通配符模式查找。直接调用即可，不需要先调用 `get_full_text`。
...
| 类型 | 说明 | Action 编号 |
|------|------|-------------|
| **查询类** | 直接调用，无需先查范围。`find_all` 等全文搜索方法不需要先调用 `get_full_text` | 1-13, 47, 52, 79-82 |
| **操作类** | 写入类，需要 rng 参数（系统自动注入，无需手动填写） | 其余编号 |

**常见错误**：同时调用 `get_full_text` 和 `find_all`（`find_all` 不需要先 `get_full_text`）。
